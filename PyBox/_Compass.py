...
class Compass():
    ...
    window_pos = (1200,400)
    ini = IniHandler(os.path.join(os.path.normpath(os.path.dirname(os.path.abspath(__file__))), 'Compass +.ini'))
    config_loaded = False
    ...
    def Draw(self):
        ...
        if self.imgui.begin("Py4GW Minimap",  flags):
            ...
            if self.position.clicked_size > 0:
                ...
                self.imgui.draw_list_add_circle(x, y, 5 + self.position.clicked_size/10, Utils.RGBToColor(255, 255, 0, int(255 - self.position.clicked_size)), 24, 2)
    ...
    def CheckClick(self):
        if self.imgui.is_mouse_clicked(0): 
            if self.imgui.get_io().key_ctrl:
                ...
                if math.dist([pos.x, pos.y], [self.position.current_pos.x, self.position.current_pos.y]) > self.position.current_size*1.1:
                    return
...
def DrawConfig():
    ...
    if PyImGui.collapsing_header(f'Compass'):
        # position settings
        if PyImGui.tree_node(f'Position'):
            header_opened = True
        ...
        # range ring settings
        if PyImGui.tree_node(f'Range Rings'):
        ...
        if PyImGui.tree_node(f'Pathing'):
        ...
        if PyImGui.button('Save Settings', 150):
            compass.SaveConfig()

...
bot_vars.loot.pickup_list    = {ModelID.Gold_Coins  : ('Gold Coins', False), 
                                ModelID.Lockpick    : ('Lockpicks',  False),
                                ModelID.Plant_Fiber : ('Fiber',      True),
                                ModelID.Dragon_Root : ('Roots',      True),
                                ModelID.Ruby        : ('Rubies',     False)}
...
def Run():
    ...
    agent_array = AgentArray.Filter.ByDistance(agent_array, Player.GetXY(), 1050)

    if agent_array and bot_funcs.Skills.IsRecharged(Build.sf):
        yield from bot_funcs.Skills.CastSkill([Build.dp, Build.sf])

    if bot_funcs.Skills.HasEffect(Build.sf) and not bot_funcs.Skills.HasEffect(Build.ns):
        yield from bot_funcs.Skills.CastSkill(Build.ns)
...
def Prep():
    yield from bot_funcs.Skills.CastSkill([Build.sod, Build.ds])
    yield from wait(.5)
    yield from bot_funcs.Skills.CastSkill(Build.ns)

def Kill():
    yield from bot_funcs.Skills.CastSkill(Build.w)
    while not bot_funcs.Skills.IsRecharged(Build.sf) or not bot_funcs.Skills.IsRecharged(Build.ds):
        if Agent.IsDead(Player.GetAgentID()):
            return
        yield from wait(.5)
    yield from bot_funcs.Skills.CastSkill([Build.sf, Build.ds])
    
    agent_array = AgentArray.GetEnemyArray()
    agent_array = AgentArray.Sort.ByDistance(agent_array, (-5832,16688))
    if len(agent_array) > 0:
        yield from bot_funcs.Agents.ChangeTarget(agent_array[0])
        yield from wait(.5)

    yield from bot_funcs.Skills.CastSkill(Build.dc)
    while bot_funcs.Skills.IsRecharged(Build.dc):
        if Agent.IsDead(Player.GetAgentID()):
            return
        yield from wait(.1)
...
def BotLogic():
    ...
    while True:
        ...
        if (bot_vars.do_setup or Map.GetMapID() != Maps.sas) and not bot_vars.handle_inv:
            ...
            yield from bot_funcs.Maps.Travel(Maps.sas)
            yield from bot_funcs.Skills.LoadSkillBar(Build.template)
            bot_funcs.Skills.CheckRequirements({'Shadow Arts' : 16, 'Wilderness Survival' : 12})
            yield from bot_funcs.Maps.SetMode(1)
            yield from bot_funcs.Move.Zone(Path.zone, Maps.dt)
            yield from bot_funcs.Move.Zone(Path.rezone, Maps.sas)
        ...
        yield from Prep()
        yield from bot_funcs.Move.FollowPath(Path.aggro, rand = 15, do_func = Run)
        yield from wait(1)
        yield from bot_funcs.Move.FollowPath([Path.kill[0]], rand = 5)
        yield from wait(1)
        yield from bot_funcs.Move.FollowPath([Path.kill[1]], rand = 5)
        ...
        yield from bot_funcs.Maps.ResignAndReturn(Maps.sas)
# endregion

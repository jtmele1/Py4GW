...
class SynchronousRoutines:
    ...
    class Move:
        ...
        @staticmethod
        def Zone(path, map_id):
            if SynchronousRoutines.log_move or SynchronousRoutines.log_all:
                Log(f'Zoning to {Map.GetMapName(map_id)} - MapID [{map_id}].')

            SynchronousRoutines.Move.FollowPath(path)
    ...
    class Agents:
        ...
        def Interact(distance = None, frame_alias = '', timeout = 5000):
            ...
            # wait for window to open
                ...
                filename = r'D:\Games\Guild Wars\Py4GW_me\my_bots\synchronous\frame_aliases.json'
...
class AsyncRoutines:
    ...
    def ActionIsPending():
        global bot_vars
        if AsyncRoutines.action_check != 0 and AsyncRoutines.action_timer.GetElapsedTime() > 0:
            if AsyncRoutines.action_timer.HasElapsed(AsyncRoutines.action_check):
                AsyncRoutines.action_check = 0
                AsyncRoutines.action_timer.Stop()
                return False
        if AsyncRoutines.action_check == 0 and AsyncRoutines.action_timer.GetElapsedTime() == 0:
            return False
        return True
    ...
    def SetPendingAction(time: float = 1000):
        global bot_vars
        AsyncRoutines.action_check = time
        AsyncRoutines.action_timer.Reset()

    class Move:
        @staticmethod
        def FollowPath(path_handler, follow_handler, enemy_check = lambda: False, enemy_handler = lambda: False):
        ...
        @staticmethod
        def PathFinished(path_handler, follow_handler, map_id = None):
            if Agent.IsDead(Player.GetAgentID()):
                #follow_handler.reset()
                return True
            if Map.IsMapLoading():
                #follow_handler.reset()
                return True
            if map_id and Map.GetMapID() == map_id:
                #follow_handler.reset()
                return True
    ...
    class Maps:
        ...
        @staticmethod
        def Arrived(map_id, outpost = 1):
            if Map.IsMapReady() and Map.GetMapID() == map_id and Party.IsPartyLoaded():
                if (outpost and Map.IsOutpost()) or (not outpost and Map.IsExplorable()):
                    Log(f'Arrived at {Map.GetMapName(map_id)} - MapID [{map_id}].')
                    return True
            return False
    ...
    class Skills:
        ...
        @staticmethod
        def CastSkill(slot, wait_for_aftercast = True):
            global bot_vars
        ...
        def GetAftercast(slot):
            ...
            activation = Skill.Data.GetActivation(skill_id)
            aftercast = Skill.Data.GetAftercast(skill_id)    
            return max(activation*1000 + aftercast*1000 + Py4GW.PingHandler().GetCurrentPing() + 50,200)
